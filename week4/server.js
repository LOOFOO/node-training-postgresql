  ...
  if (req.url === "/api/credit-package" && req.method === "GET") {
    ...
    req.on("end", async () => {
      try {
        ...
        const coachesSkillRepo = await AppDataSource.getRepository("Skill")
        const existSkill = await coachesSkillRepo.find({
          where: {
            name: data.name
          }
        })
        ...
        const newSkill = await coachesSkillRepo.create({
          name: data.name
        })
        const result = await coachesSkillRepo.save(newSkill)
        res.writeHead(200, headers)
        res.write(JSON.stringify({
          status: "success",
          data: result
